...
angular
  ...
  .controller('UsersController', function UsersController($scope, $state, $http, $window){
    ...
    function updateUser(){
      $http
        .put('https://localhost:5000/user', self.editUser)
    ...
    function loginUser(){
      $http
        .post('https://localhost:5000/user/auth', self.getUser)
        .then(function(response){
          if (response.data.success) {
            $window.localStorage.token = response.data.token;
    ...
    function deleteUser() {
      $http
        .delete('https://localhost:5000/user')
        .then(function(){
          logoutUser();
        });
    }
  ...
  .controller('ArticlesController', function ArticlesController($rootScope, $state, $http, $window){
    ...
    function getArticles(){
      $http
        .get('https://localhost:5000/article/' + $window.localStorage.user)
        .then(function(response){
    ...
    function addArticle(){
      ...
      $http
        .post('https://localhost:5000/article/' + $window.localStorage.user, self.newArticle)
        .then(function(response){
    ...
    function deleteArticle(id){
      $http({
        url: 'https://localhost:5000/article/' + id,
        method: "DELETE"
      })
      .then(function(response){
        $window.location.reload();
